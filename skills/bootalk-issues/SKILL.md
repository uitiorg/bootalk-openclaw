...
lookup.py 검색 결과를 바탕으로 확신도를 판단:

**확신도 높음** (검색 결과에서 컴포넌트/API 매핑이 명확히 나옴):
→ Step 3~4를 실행하고 콜백 메시지 응답:
```
이슈 #{번호} 등록 완료
...
**확신도 낮음** (검색 결과 없음, 모호함, 또는 레포 판단 불가):
→ **이슈를 생성하지 않는다.** 잘못된 레포에 이슈가 만들어지는 것을 방지.
→ 대신 추가 정보를 요청하는 메시지를 보낸다:
```
이슈 등록을 위해 추가 정보가 필요합니다.

설명: {유저 원문}
현재 상태: 관련 화면/컴포넌트를 특정할 수 없습니다.

다음 중 해당하는 정보를 알려주세요:
• 어떤 화면에서 발생하나요? (예: 매물 상세, 지도, 단지 브리핑)
• 어떤 동작 중에 발생하나요? (예: 페이지 로딩, 버튼 클릭, 검색)
• 에러 메시지나 스크린샷이 있나요?
```
유저가 추가 정보를 제공하면 다시 Step 1부터 분석을 시도한다.
